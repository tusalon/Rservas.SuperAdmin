            ...
            <div class="bg-white rounded-lg shadow border-l-4 ${ec.color} p-4 fade-in">
                <div class="flex flex-col md:flex-row justify-between items-start gap-3">
                    ...
                    <div class="relative">
                        ...
                        <div id="menu-${n.id}" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl z-50 border">
                            <div class="py-1">
                                ${n.estado_suscripcion === 'trial' ? `<button onclick="activarDesdeTrial('${n.id}', '${n.nombre.replace(/'/g, "\\'")}')" class="block w-full text-left px-4 py-2 hover:bg-green-50 text-green-600 text-sm">✅ Activar</button>` : ''}
...
async function exportarCSV() {
    ...
    const headers = ['ID', 'Nombre', 'Email', 'Teléfono', 'Estado', 'Reservas Mes', 'Profesionales', 'Días Activo', 'Próximo Pago', 'Monto', 'Días para Renovar'];
    const rows = resultados.map(n => [
        n.id, 
        n.nombre || '', 
        n.email || '', 
            ...
            <div class="bg-white rounded-lg shadow border-l-4 ${ec.color} p-4 fade-in">
                <div class="flex flex-col md:flex-row justify-between items-start gap-3">
                    ...
                    <div class="relative">
                        ...
                        <div id="menu-${negocioId}" class="hidden absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-xl z-50 border menu-transition">
                            <div class="py-1">
